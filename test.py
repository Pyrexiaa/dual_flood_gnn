...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--model", type=str, required=True, help="Model to use for validation"
    )
    parser.add_argument(
        "--model_path", required=True, default=None, help="Path to trained model file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug", type=bool, default=False, help="Add debug messages to output"
    )
...
def run_test(
    ...
    if output_dir is not None:
        ...
        model_filename = os.path.splitext(os.path.basename(model_path))[
            0
...
def main():
    ...
    try:
        ...
        current_device = (
            torch.cuda.get_device_name(args.device) if args.device != "cpu" else "CPU"
        )
