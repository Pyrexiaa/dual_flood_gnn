...
class InMemoryFlood1D2DDataset(FloodEvent1D2DDataset):
    ...
    def load_to_memory(self) -> List[Data]:
        ...
        t_2d_edge_index = torch.from_numpy(edge_index.copy()).long()
        t_1d_edge_index = torch.from_numpy(edge_index_1d.copy()).long()
        t_1d2d_edge_index = torch.from_numpy(edge_index_1d_2d.copy()).long()
        ...
        for idx in progress_bar:
            ...
            data = Data1D2D(x=node_2d_features,
                    edge_index=t_2d_edge_index,
                    edge_attr=edge_2d_features,
                    y=label_2d_nodes,
                    y_edge=label_2d_edges,
                    x_1d=node_1d_features,
                    edge_index_1d=t_1d_edge_index,
                    edge_attr_1d=edge_1d_features,
                    y_1d=label_1d_nodes,
                    y_1d_edge=label_1d_edges,
                    edge_index_1d_2d=t_1d2d_edge_index,
                    timestep=timestep,
                    global_mass_info=global_mass_info,
                    local_mass_info=local_mass_info)
            
            data.num_nodes = node_2d_features.size(0)
            data.num_nodes_1d = node_1d_features.size(0)
