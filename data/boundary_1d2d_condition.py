...
class Boundary1d2dCondition:
    ...
    def _init(self) -> None:
        min_elevation = get_min_cell_elevation(self.hec_ras_path, perimeter_name=self.perimeter_name)
    ...
    def remove(
        ...
        if not self._is_called["create"]:
            print("WARNING: Attempting to remove boundary condition before it has been created.")
        ...
        all_boundary_nodes = np.concatenate([
            np.array(self.init_inflow_boundary_nodes) if self.init_inflow_boundary_nodes else np.array([], dtype=int),
            np.array(self.init_outflow_boundary_nodes) if self.init_outflow_boundary_nodes else np.array([], dtype=int)
        ])
        ...
        ghost_edges_mask_original = np.any(np.isin(edge_index, ghost_nodes_to_ignore), axis=0)
        ...
        for i in range(min(5, self._boundary_edge_index.shape[1])):
            print(f"    {self._boundary_edge_index[0, i]} -> {self._boundary_edge_index[1, i]}")
        ...
        self._ghost_nodes_to_ignore = np.array([], dtype=int)  # No ghosts in filtered data
        ...
        return static_nodes_filtered, dynamic_nodes_filtered, static_edges_filtered, dynamic_edges_filtered, edge_index_remapped
    ...
    def apply(
        ...
        if self._boundary_edge_index is not None and self._boundary_edge_index.shape[1] > 0:
            ...
            dynamic_edges = np.concatenate([dynamic_edges, self._boundary_dynamic_edges], axis=1)
            ...
            boundary_edges_valid_mask = np.ones(self._boundary_edge_index.shape[1], dtype=bool)
            self._valid_edges_mask = np.concatenate([self._valid_edges_mask, boundary_edges_valid_mask])
        ...
        print("\nMasks created:")
        print(f"  boundary_nodes_mask: {self.boundary_nodes_mask.shape}, True count: {np.sum(self.boundary_nodes_mask)}")
        print(f"  Boundary node indices (ORIGINAL): {all_boundary_nodes}")
        print(f"  boundary_edges_mask: {self.boundary_edges_mask.shape}, True count: {np.sum(self.boundary_edges_mask)}")
        print(f"  inflow_edges_mask: {self.inflow_edges_mask.shape}, True count: {np.sum(self.inflow_edges_mask)}")
        print(f"  outflow_edges_mask: {self.outflow_edges_mask.shape}, True count: {np.sum(self.outflow_edges_mask)}")
    ...
    def _create_masks(self, new_num_nodes: int, edge_index: ndarray) -> None:
        ...
        if self.init_outflow_boundary_nodes and len(self.init_outflow_boundary_nodes) > 0:
