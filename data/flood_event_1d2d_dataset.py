...
class FloodEvent1D2DDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.boundary_condition = Boundary1d2dCondition(root_dir=root_dir,
    ...
    def raw_file_names(self):
        return [self.nodes_1d_shp_file, self.edges_1d_shp_file, self.edges_1d2d_shp_file, self.nodes_2d_shp_file, self.edges_2d_shp_file, self.dem_file, *self.hec_ras_files]
    ...
    def process(self):
        ...
        if self.is_normalized:
            ...
            # 1D Normalization
            static_nodes_1d = self.normalizer.normalize_feature_vector(self.STATIC_1D_NODE_FEATURES, static_nodes_1d)
            dynamic_nodes_1d = self.normalizer.normalize_feature_vector(self.DYNAMIC_1D_NODE_FEATURES, dynamic_nodes_1d)
            static_edges_1d = self.normalizer.normalize_feature_vector(self.STATIC_1D_EDGE_FEATURES, static_edges_1d)
            dynamic_edges_1d = self.normalizer.normalize_feature_vector(self.DYNAMIC_1D_EDGE_FEATURES, dynamic_edges_1d)
       ...
       # Save Constant Values
        ...
        if self.save:
            saved_output_dir = os.path.join(self.processed_dir, "features_csv", self.mode)
    ...
    def get(self, idx):
        ...
        dynamic_nodes_1d: ndarray = dynamic_values['dynamic_nodes_1d'].astype(np.float32)
        dynamic_edges_1d: ndarray = dynamic_values['dynamic_edges_1d'].astype(np.float32)
        ...
        node_features = self._get_2d_node_timestep_data(static_nodes, dynamic_nodes, within_event_idx)
        edge_features = self._get_2d_edge_timestep_data(static_edges, dynamic_edges, within_event_idx)
        
        # 1D Data Construction
        node_features_1d = self._get_1d_node_timestep_data(static_nodes_1d, dynamic_nodes_1d, within_event_idx)
        edge_features_1d = self._get_1d_edge_timestep_data(static_edges_1d, dynamic_edges_1d, within_event_idx)

        label_nodes, label_edges, label_1d_nodes, label_1d_edges = self._get_timestep_labels(dynamic_nodes, dynamic_edges, dynamic_nodes_1d, dynamic_edges_1d, within_event_idx)
    ...
    def _get_dynamic_1d_node_features(self) -> ndarray:
        DYNAMIC_1D_NODE_RETRIEVAL_MAP = {
            "water_level": lambda: self._get_dynamic_from_all_unique_1d_events(get_1d_water_level, aggr='mean'),
            "inlet_flow": lambda: self._get_dynamic_from_all_unique_1d_events(get_1d_inlet_flow, aggr='mean'),
        }
        dynamic_features = self._get_features(feature_list=self.DYNAMIC_1D_NODE_FEATURES,
    ...
    def _get_dynamic_1d_edge_features(self) -> ndarray:
        DYNAMIC_1D_EDGE_RETRIEVAL_MAP = {
            "velocity": lambda: self._get_dynamic_from_all_unique_1d_events(get_1d_velocity, aggr='mean'),
            "flow": lambda: self._get_dynamic_from_all_unique_1d_events(get_1d_flow, aggr='mean'),
        }
        dynamic_features = self._get_features(feature_list=self.DYNAMIC_1D_EDGE_FEATURES,
    ...
    def _get_static_node_features(self) -> ndarray:
        ...
        STATIC_NODE_RETRIEVAL_MAP = {
            ...
            "flow_accumulation": lambda: _get_flow_accumulation(self.raw_paths[3], self.raw_paths[5]),
        }
    ...
    def _get_static_edge_features(self) -> ndarray:
        ...
        STATIC_EDGE_RETRIEVAL_MAP = {
            ...
            "relative_position_x": lambda: get_relative_position('x', self.raw_paths[3], self.raw_paths[4]),
            "relative_position_y": lambda: get_relative_position('y', self.raw_paths[3], self.raw_paths[4]),
        }
    ...
    def _get_dynamic_node_features(self) -> ndarray:
        ...
        def get_water_depth(hec_ras_path: str):
            ...
            water_level = get_water_level(hec_ras_path, perimeter_name=self.perimeter_name)
        ...
        DYNAMIC_NODE_RETRIEVAL_MAP = {
            ...
            "water_level": lambda: self._get_dynamic_from_all_unique_events(get_water_level, aggr='mean'),
        }
    ...
    def _get_dynamic_edge_features(self) -> ndarray:
        DYNAMIC_EDGE_RETRIEVAL_MAP = {
            "velocity": lambda: self._get_dynamic_from_all_unique_events(get_velocity, aggr='mean'),
            "flow": lambda: self._get_dynamic_from_all_unique_events(get_face_flow, aggr='mean'),
        }
    ...
    def _get_dynamic_from_all_unique_events(self, retrieval_func: Callable, aggr: str = 'first') -> ndarray:
        all_event_data = []
        for i, hec_ras_path in enumerate(self.raw_paths[6:]):
            event_data = retrieval_func(hec_ras_path, perimeter_name=self.perimeter_name)
            event_data = self._get_trimmed_dynamic_data(event_data, i, aggr)
            all_event_data.append(event_data)
        all_event_data = np.concatenate(all_event_data, axis=0)
        return all_event_data
    
    def _get_dynamic_from_all_unique_1d_events(self, retrieval_func: Callable, aggr: str = 'first') -> ndarray:
        all_event_data = []
        for i, hec_ras_path in enumerate(self.raw_paths[6:]):
    ...
    def _get_1d_node_timestep_data(self, static_features: ndarray, dynamic_features: ndarray, timestep_idx: int) -> Tensor:
        ts_dynamic_features = self._get_timestep_dynamic_features(dynamic_features, self.DYNAMIC_1D_NODE_FEATURES, timestep_idx)
    ...
    def _get_1d_edge_timestep_data(self, static_features: ndarray, dynamic_features: ndarray, timestep_idx: int) -> Tensor:
        ts_dynamic_features = self._get_timestep_dynamic_features(dynamic_features, self.DYNAMIC_1D_EDGE_FEATURES, timestep_idx)
        return self._get_timestep_features(static_features, ts_dynamic_features)
    ...
    def _get_timestep_labels(
        ...
        # (num_nodes, 1)
        assert timestep_idx + 1 < node_dynamic_features.shape[0], "Attempting to access node label beyond available timesteps"
        ...
        # (num_edges, 1)
        assert timestep_idx + 1 < edge_dynamic_features.shape[0], "Attempting to access edge label beyond available timesteps"
        ...
        # (num_nodes, 1)
        assert timestep_idx + 1 < node_1d_dynamic_features.shape[0], "Attempting to access node label beyond available timesteps"
        current_1d_nodes = node_1d_dynamic_features[timestep_idx, :, label_1d_nodes_idx][:, None]
        next_1d_nodes = node_1d_dynamic_features[timestep_idx+1, :, label_1d_nodes_idx][:, None]
        ...
        # (num_edges, 1)
        assert timestep_idx + 1 < edge_1d_dynamic_features.shape[0], "Attempting to access edge label beyond available timesteps"
        current_1d_edges = edge_1d_dynamic_features[timestep_idx, :, label_1d_edges_idx][:, None]
        next_1d_edges = edge_1d_dynamic_features[timestep_idx+1, :, label_1d_edges_idx][:, None]
    ...
    def save_features_to_csv(self, output_dir: str = None):
        ...
        edge_index_2d_df = pd.DataFrame(edge_index_2d.T, columns=["from_node", "to_node"])
        ...
        edge_index_1d_df = pd.DataFrame(edge_index_1d.T, columns=["from_node", "to_node"])
        ...
        edge_index_1d_2d_df.to_csv(
            os.path.join(output_dir, "1d2d_connections.csv"), index_label="connection_idx"
        )
        ...
        for event_idx, run_id in enumerate(self.hec_ras_run_ids):
            ...
            timesteps_df.to_csv(
                os.path.join(event_dir, 'timesteps.csv'),
                index=False
            )
            ...
            dynamic_nodes_2d_flat = dynamic_nodes_2d.reshape(-1, len(self.DYNAMIC_NODE_FEATURES))
            dynamic_nodes_2d_df = pd.DataFrame(
                dynamic_nodes_2d_flat,
                columns=self.DYNAMIC_NODE_FEATURES
            )
            dynamic_nodes_2d_df['timestep'] = np.repeat(np.arange(num_timesteps), num_2d_nodes)
            dynamic_nodes_2d_df['node_idx'] = np.tile(np.arange(num_2d_nodes), num_timesteps)
            ...
            dynamic_nodes_2d_df.to_csv(
                os.path.join(event_dir, '2d_nodes_dynamic_all.csv'),
                index=False
            )
            
            # --- 2D Edges: All timesteps in one file ---
            dynamic_edges_2d_flat = dynamic_edges_2d.reshape(-1, len(self.DYNAMIC_EDGE_FEATURES))
            dynamic_edges_2d_df = pd.DataFrame(
                dynamic_edges_2d_flat,
                columns=self.DYNAMIC_EDGE_FEATURES
            )
            dynamic_edges_2d_df['timestep'] = np.repeat(np.arange(num_timesteps), num_2d_edges)
            dynamic_edges_2d_df['edge_idx'] = np.tile(np.arange(num_2d_edges), num_timesteps)
            cols = ['timestep', 'edge_idx'] + self.DYNAMIC_EDGE_FEATURES
            dynamic_edges_2d_df = dynamic_edges_2d_df[cols]
            dynamic_edges_2d_df.to_csv(
                os.path.join(event_dir, '2d_edges_dynamic_all.csv'),
                index=False
            )
            
            # --- 1D Nodes: All timesteps in one file ---
            dynamic_nodes_1d_flat = dynamic_nodes_1d.reshape(-1, len(self.DYNAMIC_1D_NODE_FEATURES))
            dynamic_nodes_1d_df = pd.DataFrame(
                dynamic_nodes_1d_flat,
                columns=self.DYNAMIC_1D_NODE_FEATURES
            )
            dynamic_nodes_1d_df['timestep'] = np.repeat(np.arange(num_timesteps), num_1d_nodes)
            dynamic_nodes_1d_df['node_idx'] = np.tile(np.arange(num_1d_nodes), num_timesteps)
            ...
            dynamic_nodes_1d_df.to_csv(
                os.path.join(event_dir, '1d_nodes_dynamic_all.csv'),
                index=False
            )
            
            # --- 1D Edges: All timesteps in one file ---
            dynamic_edges_1d_flat = dynamic_edges_1d.reshape(-1, len(self.DYNAMIC_1D_EDGE_FEATURES))
            dynamic_edges_1d_df = pd.DataFrame(
                dynamic_edges_1d_flat,
                columns=self.DYNAMIC_1D_EDGE_FEATURES
            )
            dynamic_edges_1d_df['timestep'] = np.repeat(np.arange(num_timesteps), num_1d_edges)
            dynamic_edges_1d_df['edge_idx'] = np.tile(np.arange(num_1d_edges), num_timesteps)
            ...
            dynamic_edges_1d_df.to_csv(
                os.path.join(event_dir, '1d_edges_dynamic_all.csv'),
                index=False
            )
            
            self.log_func(f'  Saved event {run_id}: {num_timesteps} timesteps (all in single files)')
