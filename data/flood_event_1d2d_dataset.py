...
class FloodEvent1D2DDataset(Dataset):
    ...
    def get(self, idx):
        ...
        node_features = self._get_node_timestep_data(static_nodes, dynamic_nodes, within_event_idx, self.DYNAMIC_NODE_FEATURES)
        edge_features = self._get_edge_timestep_data(static_edges, dynamic_edges, within_event_idx, self.DYNAMIC_EDGE_FEATURES)
        label_nodes, label_edges = self._get_timestep_labels(dynamic_nodes, dynamic_edges, within_event_idx, self.DYNAMIC_NODE_FEATURES, self.DYNAMIC_EDGE_FEATURES, self.NODE_TARGET_FEATURE, self.EDGE_TARGET_FEATURE)

        # 1D Data Construction
        node_features_1d = self._get_node_timestep_data(static_nodes_1d, dynamic_nodes_1d, within_event_idx, self.DYNAMIC_1D_NODE_FEATURES)
        edge_features_1d = self._get_edge_timestep_data(static_edges_1d, dynamic_edges_1d, within_event_idx, self.DYNAMIC_1D_EDGE_FEATURES)
    ...
    def _get_hecras_files_from_summary(self, root_dir: str, dataset_summary_file: str) -> Tuple[List[str], List[str]]:
        ...
        print("HecRas Files:", hec_ras_files)
        print("HecRas Run IDs:", hec_ras_run_ids)
    ...
    def _get_node_timestep_data(self, static_features: ndarray, dynamic_features: ndarray, timestep_idx: int, dynamic_feature_list: List[str]) -> Tensor:
        ts_dynamic_features = self._get_timestep_dynamic_features(dynamic_features, dynamic_feature_list, timestep_idx)
        return self._get_timestep_features(static_features, ts_dynamic_features)

    def _get_edge_timestep_data(self, static_features: ndarray, dynamic_features: ndarray, timestep_idx: int, dynamic_feature_list: List[str]) -> Tensor:
        ts_dynamic_features = self._get_timestep_dynamic_features(dynamic_features, dynamic_feature_list, timestep_idx)
        return self._get_timestep_features(static_features, ts_dynamic_features)
    ...
    def _get_timestep_labels(
            ...
            edge_feature_list: List[str],
            node_target: str, 
            edge_target: str,
        ) -> Tuple[Tensor, Tensor]:
        label_nodes_idx = node_feature_list.index(node_target)
        # (num_nodes, 1)
        assert timestep_idx + 1 < len(node_feature_list), "Attempting to access node label beyond available timesteps"
        ...
        label_edges_idx = edge_feature_list.index(edge_target)
        # (num_edges, 1)
        assert timestep_idx + 1 < len(edge_feature_list), "Attempting to access edge label beyond available timesteps"
