...
class HeteroNodeEdgeAutoregressive1D2DTester(Base1D2DTester):
    ...
    def test(self):
        for event_idx, run_id in enumerate(self.dataset.hec_ras_run_ids):
            ...
            self.run_test_for_event(
                event_idx,
                validation_stats,
                predictions_dir=f"./kaggle_submissions/{self.dataset.model_name}_{self.dataset.previous_timesteps}",
            )
    ...
    def run_test_for_event(
        ...
        with torch.no_grad():
            ...
            for graph in dataloader:
                ...
                # 2D features
                hetero_data = self._create_hetero_data_for_inference(
                    graph,
                    sliding_window,
                    sliding_window_1d,
                    edge_sliding_window,
                ...
                pred_diff = pred_dict["2d"]
                pred_diff_1d = pred_dict["1d"]
                ...
                if self.include_physics_loss:
                    prev_edge_pred = physics_utils.get_curr_flow_from_edge_features(
                        hetero_data["2d", "to", "2d"].edge_attr,
                        self.dataset.previous_timesteps,
                    )
