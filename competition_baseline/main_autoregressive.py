...
from dataset import CombinedDataset, WaterLevelDataset1D, WaterLevelDataset2D, SequenceDataset, collate_sequences
...
def train_autoregressive(
    ...
    input_dim = train_combined_base.dataset_1d.dataset.X.shape[-1]
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (X_seq, y_seq, node_type, node_id, timesteps, event_id, seq_lengths) in enumerate(train_loader):
            ...
            for b in range(batch_size_actual):
                ...
                # Get this sequence
                X_current = X_seq[b, 0, :, :].unsqueeze(0)  # (1, window, features) - first window
                    ...
                    if step < seq_len - 1:
                        ...
                        if step + 1 < X_seq.shape[1]:
                            ...
                            if use_gt:
                                ...
                                next_timestep[0, 0, water_level_idx] = pred.squeeze().detach()
                            next_window = torch.cat([X_current[:, 1:, :], next_timestep], dim=1)
        ...
        train_rmse = (train_loss_sum / train_samples) ** 0.5 if train_samples > 0 else 0.0
        ...
        with torch.no_grad():
            for X_seq, y_seq, node_type, node_id, timesteps, event_id, seq_lengths in test_loader:
        ...
        print(f"Epoch {epoch+1:03d}/{epochs} [TF: {current_tf_ratio:.3f}] Train RMSE: {train_rmse:.4f}, Val RMSE: {val_rmse:.4f}")
...
if __name__ == "__main__":
    ...
    if train_bool:
        ...
        model, train_ds, test_ds, trained_normalizer_1d, trained_normalizer_2d = train_autoregressive(
            train_events,
            test_events,
            window=5,
            max_events=None,
