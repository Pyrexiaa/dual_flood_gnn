...
import time
import numpy as np
from testing import HeteroNodeEdgeAutoregressive1D2DTester
import torch
...
class HeteroNodeEdgeAutoregressive1D2DTrainer(
    ...
    def _convert_to_hetero_data(self, batch, timestep: int = 0) -> HeteroData:
        ...
        # 2D node features
        hetero_data["2d"].x = batch.x[:, :, timestep].float()
        hetero_data["2d"].num_nodes = batch.x.shape[0]

        # 1D node features
        hetero_data["1d"].x = batch.x_1d[:, :, timestep].float()
        hetero_data["1d"].num_nodes = batch.x_1d.shape[0]

        # 2D-2D edges
        hetero_data["2d", "connects", "2d"].edge_index = batch.edge_index
        hetero_data["2d", "connects", "2d"].edge_attr = batch.edge_attr[
            :, :, timestep
        ].float()

        # 1D-1D edges
        hetero_data["1d", "connects", "1d"].edge_index = batch.edge_index_1d
        hetero_data["1d", "connects", "1d"].edge_attr = batch.edge_attr_1d[
            :, :, timestep
        ].float()
    ...
    def _train_model(
        ...
        for batch in self.dataloader:
            ...
            x_init = batch.x[:, :, 0]
            x_1d_init = batch.x_1d[:, :, 0]
            edge_attr_init = batch.edge_attr[:, :, 0]
            edge_attr_1d_init = batch.edge_attr_1d[:, :, 0]

            sliding_window = x_init[
                :, self.start_node_target_idx : self.end_node_target_idx
            ].clone()
            sliding_window_1d = x_1d_init[
                :, self.start_1d_node_target_idx : self.end_1d_node_target_idx
            ].clone()
            edge_sliding_window = edge_attr_init[
                :, self.start_edge_target_idx : self.end_edge_target_idx
            ].clone()
            edge_sliding_window_1d = edge_attr_1d_init[
                :, self.start_1d_edge_target_idx : self.end_1d_edge_target_idx
            ].clone()

            for i in range(current_num_timesteps):
                # Create HeteroData for current timestep
                hetero_data = self._convert_to_hetero_data(batch, timestep=i)

                # Update with sliding windows
                pred_dict = self.model(hetero_data)

                pred_diff = pred_dict["2d"]  # 2D node predictions
                pred_diff_1d = pred_dict["1d"]  # 1D node predictions
                ...
                if self.use_physics_loss:
                    curr_face_flow = physics_utils.get_curr_flow_from_edge_features(
                        hetero_data["2d", "to", "2d"].edge_attr, previous_timesteps
                    )
    ...
    def validate(self):
        val_tester = HeteroNodeEdgeAutoregressive1D2DTester(
            model=self.model,
