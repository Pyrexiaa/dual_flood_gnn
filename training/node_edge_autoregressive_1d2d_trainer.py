...
class NodeEdgeAutoregressive1D2DTrainer(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Edge (discharge) loss configuration
        self.edge_loss_func = edge_loss_func if edge_loss_func is not None else self.loss_func
        self.edge_loss_weight = edge_loss_weight
        self.edge_pred_loss_scale = edge_pred_loss_scale
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            (
                ...
                pred_epoch_loss,
                pred_1d_epoch_loss,
                edge_pred_epoch_loss,
                edge_pred_1d_epoch_loss,
            ...
            logging_str += f"\t2D Edge Prediction Loss: {edge_pred_epoch_loss:.4e}\n"
            logging_str += f"\t1D Edge Prediction Loss: {edge_pred_1d_epoch_loss:.4e}"
            ...
            self.training_stats.add_loss_component(
                "edge_prediction_loss_2d", edge_pred_epoch_loss
            )
            self.training_stats.add_loss_component(
                "edge_prediction_loss_1d", edge_pred_1d_epoch_loss
            )
            ...
            self.training_stats.log(f"\tValidation 1D Node RMSE: {val_1d_node_rmse:.4e}")
            self.training_stats.log(f"\tValidation 2D Edge RMSE: {val_edge_rmse:.4e}")
            self.training_stats.log(
                f"\tValidation 1D Edge RMSE: {val_1d_edge_rmse:.4e}"
            )
            ...
            self.training_stats.add_val_loss_component(
                "val_edge_rmse_2d", val_edge_rmse
            )
            self.training_stats.add_val_loss_component(
                "val_edge_rmse_1d", val_1d_edge_rmse
            )

            current_timestep_epochs += 1

            # Early-stopping metric: mean of all four target RMSEs (baseline heuristic;
            # note node water-level and edge discharge are on different scales).
            combined_rmse = (
                val_node_rmse + val_1d_node_rmse + val_edge_rmse + val_1d_edge_rmse
            ) / 4
    ...
    def _train_model(
        ...
        running_edge_pred_1d_loss = 0.0
        running_global_mass_loss = 0.0
        running_local_mass_loss = 0.0

        pbar = tqdm(
            self.dataloader,
            desc=f"Epoch {epoch + 1} [{current_num_timesteps} steps]",
            leave=False,
        )
        for batch in pbar:
            ...
            total_batch_edge_pred_1d_loss = 0.0
            ...
            for i in range(current_num_timesteps):
                ...
                # Override graph data with sliding windows (nodes AND edges)
                x = torch.concat(
                    [
                ...
                # Override boundary conditions (2D nodes and 2D edges)
                pred_diff = self._override_pred_bc(pred_diff, batch, i)
                edge_pred_diff = self._override_edge_pred_bc(edge_pred_diff, batch, i)
                ...
                pred_1d_loss = self._scale_1d_node_pred_loss(epoch, pred_1d_loss)
                ...
                edge_pred_1d_loss = self._compute_edge_1d_loss(
                    edge_pred_diff_1d, batch, i
                )
                edge_pred_1d_loss = self._scale_edge_pred_loss(epoch, edge_pred_1d_loss)
                total_batch_edge_pred_1d_loss += edge_pred_1d_loss.item()

                step_loss = (
                    pred_loss + pred_1d_loss + edge_pred_loss + edge_pred_1d_loss
                )
                ...
                if i < current_num_timesteps - 1:  # Don't update on last iteration
                    ...
                    sliding_window_1d = torch.cat(
                        (sliding_window_1d[:, 1:], pred_1d), dim=1
                    )
                    edge_sliding_window = torch.cat(
                        (edge_sliding_window[:, 1:], edge_pred), dim=1
                    )
                    edge_sliding_window_1d = torch.cat(
            ...
            pbar.set_postfix(
                loss=f"{avg_batch_loss.item():.3e}",
                node2d=f"{total_batch_pred_loss / current_num_timesteps:.2e}",
                edge2d=f"{total_batch_edge_pred_loss / current_num_timesteps:.2e}",
            )
            ...
            total_losses = (
                ...
                total_batch_edge_pred_loss,
                total_batch_edge_pred_1d_loss,
            ...
            running_edge_pred_1d_loss += avg_edge_pred_1d_loss
        ...
        running_loss = (
            ...
            + running_edge_pred_loss
            + running_edge_pred_1d_loss
    ...
    def validate(self):
        ...
        return node_rmse, node_1d_rmse, edge_rmse, edge_1d_rmse
    ...
    def _compute_edge_1d_loss(self, edge_pred: Tensor, batch, timestep: int) -> Tensor:
        label = batch.y_1d_edge[:, :, timestep]
        return self.edge_loss_func(edge_pred, label)

    def _scale_edge_pred_loss(self, epoch: int, edge_pred_loss: Tensor) -> Tensor:
        if epoch < self.num_epochs_dyn_loss:
            return self.edge_pred_loss_scale * edge_pred_loss
        return self.edge_pred_loss_scale * edge_pred_loss * self.edge_loss_weight
    ...
    def _override_edge_pred_bc(self, edge_pred: Tensor, batch, timestep: int) -> Tensor:
        if self.boundary_edges_mask is None or not np.any(self.boundary_edges_mask):
            return edge_pred
        batch_boundary_edges_mask = np.tile(self.boundary_edges_mask, batch.num_graphs)
        edge_pred[batch_boundary_edges_mask] = batch.y_edge[
            batch_boundary_edges_mask, :, timestep
        ]
        return edge_pred
