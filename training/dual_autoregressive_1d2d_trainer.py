...
# TODO: Edge autoregressive 1d2d trainer not done
# TODO: Dual Autogregressive 1d2d trainer not done
class DualAutoregressive1D2DTrainer(NodeAutoregressive1D2DTrainer, EdgeAutoregressive1D2DTrainer):
    def __init__(
        self,
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.training_stats.add_loss_component(
                "edge_prediction_loss", edge_pred_epoch_loss
            )
            ...
            is_early_stopped = self.early_stopping(
                (val_node_rmse, val_edge_rmse), self.model
            )
            is_max_exceeded = (
        ...
        self.training_stats.add_additional_info(
            "edge_scaled_loss_ratios", self.edge_loss_scaler.scaled_loss_ratio_history
        )
    ...
    def _train_model(
        ...
        for batch in self.dataloader:
            ...
            for i in range(current_num_timesteps):
                ...
                pred_diff, edge_pred_diff = self._override_pred_bc(
                    pred_diff, edge_pred_diff, batch, i
                )
                ...
                edge_pred_loss = self._scale_edge_pred_loss(
                    epoch, pred_loss, edge_pred_loss
                )
                ...
                if i < current_num_timesteps - 1:  # Don't update on last iteration
                    ...
                    next_edge_sliding_window = torch.cat(
    ...
    def validate(self):
        ...
        (
            event_node_loss_list,
            event_edge_loss_list,
            event_global_loss_list,
            event_local_loss_list,
        ) = [], [], [], []
        ...
        for event_idx in range(len(ds.hec_ras_run_ids)):
            with torch.no_grad():
                ...
                event_end_idx = (
                    ds.event_start_idx[event_idx + 1]
                    if event_idx + 1 < len(ds.event_start_idx)
                    else ds.total_rollout_timesteps
                )
                event_dataset = ds[event_start_idx:event_end_idx]
                dataloader = DataLoader(
                    event_dataset, batch_size=1, shuffle=False
                )  # Enforce batch size = 1 for autoregressive testing
                ...
                for graph in dataloader:
                    ...
                    pred_diff[self.boundary_nodes_mask] = graph.y[
                        self.boundary_nodes_mask
                    ]
                    ...
                    if self.use_global_loss:
                        global_mass_loss = self._get_global_mass_loss(
                            epoch, pred, prev_node_pred, prev_edge_pred, None, graph
                        )
                        global_loss_list.append(global_mass_loss)
                    if self.use_local_loss:
                        local_mass_loss = self._get_local_mass_loss(
                            epoch, pred, prev_node_pred, prev_edge_pred, None, graph
                        )
                    ...
                    label_edge = (
                        graph.edge_attr[:, [self.end_edge_target_idx - 1]]
                        + graph.y_edge
                    )
                    if ds.is_normalized:
                        edge_pred = ds.normalizer.denormalize(
                            ds.EDGE_TARGET_FEATURE, edge_pred
                        )
                        label_edge = ds.normalizer.denormalize(
                            ds.EDGE_TARGET_FEATURE, label_edge
                        )
        ...
        if self.use_global_loss:
            ...
            self.training_stats.add_val_loss_component(
                "val_global_mass_loss", avg_global_loss
            )

        if self.use_local_loss:
            avg_local_loss = torch.stack(event_local_loss_list).mean().item()
            self.training_stats.add_val_loss_component(
                "val_local_mass_loss", avg_local_loss
            )
    ...
    def _override_pred_bc(
        self, pred: Tensor, edge_pred: Tensor, batch, timestep: int
    ) -> Tensor:
        pred = NodeAutoregressive1D2DTrainer._override_pred_bc(self, pred, batch, timestep)
        edge_pred = EdgeAutoregressive1D2DTrainer._override_pred_bc(
            self, edge_pred, batch, timestep
        )
    ...
    def _update_loss_scaler_for_epoch(self, epoch: int):
        ...
        NodeAutoregressive1D2DTrainer._update_loss_scaler_for_epoch(self, epoch)
